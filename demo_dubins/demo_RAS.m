...
for i = 1 : length(sim_t)
    ...
    if i <=50
        ...
        % plot(g_grid.xs{1}(ind_verified), g_grid.xs{2}(ind_verified),'r.')
    else
        clvf = eval_u(g_grid,V_clvf,xs);
        i_clvf = find(~isnan(clvf));
        if all (clvf(i_clvf)<= 0.3)
            break
        end
        exrCLF.x = xs;
        exrCLF.gamma = 0.1;
...
visSetIm(g_grid,V_clvf,'b',0.3)

visSetIm(g_grid,V_reach_avoid(:,:,:,51),'k',0);
plot(xp,yp,'k*')
